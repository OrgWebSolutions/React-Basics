# Program 2: Create a Student component that takes name and grade as props. Use JSX to render multiple Student components dynamically

A React application that displays a list of students with their grades using reusable components.

## Features

- **Student Component**: A reusable component that accepts `name` and `grade` as props
- **Dynamic Rendering**: Multiple Student components are rendered dynamically using array mapping
- **Clean JSX**: Simple and readable component structure
...
```
src/
  ├── App.js                 # Main app component with student data
  ├── index.js              # Entry point
  └── components/
      └── Student.js        # Reusable Student component
public/
...
The `Student` component receives two props:
- `name`: The student's name
- `grade`: The student's grade

The `App` component maintains an array of student objects and uses `.map()` to dynamically render a `Student` component for each student in the list.
